...
int brew_launch(int argc, char** argv) {
    ...
    while (aptMainLoop())
    {
		...
		if(kPress & KEY_CPAD_DOWN) {consoleClear(); printf("\x1b[4;1HPRESSED CPAD DOWN!");}
		if(kHold & KEY_SELECT) {
			if (kPress & KEY_B)	server_change_timer_freq(&serv, 1, 1, &bottomScreen);
			if (kPress & KEY_A)	server_change_timer_freq(&serv, 1, 5, &bottomScreen);
			if (kPress & KEY_Y)	server_change_timer_freq(&serv, 1, 10, &bottomScreen);
			if (kPress & KEY_X)	server_change_timer_freq(&serv, 1, 80, &bottomScreen);
